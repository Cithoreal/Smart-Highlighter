# open example_report.md and assign to a variable
example_report = ""
with open("llm_judge/example_report.md", "r", encoding="utf-8") as f:
...
# For text with multiple bullet points, go through each rubric category, assign a score **1–4** (whole numbers only) and give a one‑sentence justification.
# Go through each rubric category, assign a score **1–4** (whole numbers only), and give a one‑sentence justification.  For text with multiple bullet points, give multiple sentence justification (one for each bullet point).
# Apply the rubric to entire numbered sections. For sections with multiple bullet points, give comments on each bullet point explaining why it contributes to the score of the section.
